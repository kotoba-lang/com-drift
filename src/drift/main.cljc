(ns drift.main
  "Kotodama WASM entrypoint for the Drift clean-room actor (L4) — Clojure port.
  ...
  facts (`drift.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"drift.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "drift-compat" :tier tier :entities entities} 200])
